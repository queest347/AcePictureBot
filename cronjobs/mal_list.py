...
""" This script will automatically run 6 weeks into the current anime season
    and target last season's anime.
It will scrape the MyAnimeList season page,
    check if the anime is popular (30,000+ "members").
If it passes it will then go through each character filtering
    out people without an image and keeping ones more than 20+ fans.
The last check is to see if the person has any images.
If the name doesn't have images it will put it into a
    side list for manual check up later.
"""
def printf(*objects, sep=' ', end='\n', file=sys.stdout):
    """ Safe way to print every message. """
    enc = file.encoding
    if enc == 'UTF-8':
...
def start(url):
    ...
    with Browser() as browser:
        ...
        for b in airing_div.find_all('a', class_="link-title")[3:]:
